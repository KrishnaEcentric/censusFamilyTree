...
var publicDeathRegistration = (function () {
    "use strict";
     ...
     */
    function _baseURL() {
        return dcrc_lib.baseUrl() + 'publicDeathRegistration/';
    }
    ...
    function validateCidSrpNoOnly() {
        ...
        $('#btnSearch').on('click', function () {
            $('#changeSpouseInfoDetails').prop('hidden',false);
            $('#publicRegistrationForm').validate({
    ...
    function saveResubmission() {
        $('#saveResubmit').on('click', function () {
            $('#publicdeathRegistrationDetailsResubmitForm').validate({
                submitHandler: function (form) {
                    if (isSubmitted) {
    ...
    //function getTshogpaVillageDropdownList() {
    //    $('#gewogOfDeath').on('change', function () {
    //        var gewogOfDeath = $(this).val();
    //        $.ajax({
    //            url: _baseURL() + 'getTshogpaVillageDropdownList',
    //            type: 'GET',
    //            data: {gewogID: gewogOfDeath},
    //            success: function (res) {
    //                dcrc_lib.loadDropDown($('#tshokpaVillage'), res, 'integer');
    //            }
    //        });
    //    });
    //}
    ...
    function save() {
        $('#btnSave').on('click', function () {
            $('#publicDeathRegistrationDetailsForm').validate({
                submitHandler: function (form) {
                    if (isSubmitted) {
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            if (res.status === 1) {
                                successMsgRedirect(res.text, 'publicDeathRegistration');
                            } else if (res.status == 5) {
                                warningMsg(res.text);
                            } else {
                                errorMsg(res.text)
    ...
    return {
        ...
        //getTshogpaVillageDropdownList: getTshogpaVillageDropdownList,
...
$(document).ready(function () {
    ...
    publicDeathRegistration.validateCidSrpNoOnly();
    publicDeathRegistration.nextTab();
    publicDeathRegistration.previousTab();
    publicDeathRegistration.validateCauseOfDeath();
    publicDeathRegistration.validateCountryOfDeath();
    publicDeathRegistration.validatePlaceOfDeath();
    publicDeathRegistration.validateAttendedType();
    publicDeathRegistration.validateRelationOther();
    publicDeathRegistration.getGewogDropdownList();
    publicDeathRegistration.getVillageDropdownList();
    //publicDeathRegistration.getTshogpaVillageDropdownList();
    publicDeathRegistration.getPersonAttendedDetails();
    publicDeathRegistration.getRequestorDetails();
    publicDeathRegistration.getFatherDetails();
    publicDeathRegistration.getMotherDetails();
    //publicDeathRegistration.addMoreDocuments();
    publicDeathRegistration.removeFileRow();
    publicDeathRegistration.save();
    publicDeathRegistration.removeErrorClass();
    publicDeathRegistration.getIndividualInformation();
    publicDeathRegistration.getMainCidDetails();
    publicDeathRegistration.saveResubmission();
    publicDeathRegistration.deleteDocument();
    publicDeathRegistration.getApplicationDetails();
});
