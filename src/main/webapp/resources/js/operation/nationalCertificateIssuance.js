 ...
 */
var nationalCertificateIssuanceMsg = {
    processingFormData: 'Your form request is processing. Please wait...'
};

var nationalCertificateIssuance = (function () {
    "use strict";
     ...
     */
    function _baseURL() {
        return dcrc_lib.baseUrl() + 'nationalCertificateIssuance/';
    }
    ...
    function validateCidSrpNoOnly() {
        $('#btnSearch').on('click', function () {
            $('#nationalCertificateIssuanceForm').validate({
    ...
    function save() {
        $('#btnSave').on('click', function () {
            $('#nationalCertificateIssuanceDetailsForm').validate({
                submitHandler: function (form) {
                    if (isSubmitted) {
                        warningMsg(nationalCertificateIssuanceMsg.processingFormData);
                        return;
                    }
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            if (res.status === 1) {
                                successMsgRedirect(res.text,'nationalCertificateIssuance/generatePage?applicationNumber='+res.value);
                            } else if (res.status == 5) {
                                warningMsg(res.text);
                            } else {
                                errorMsg(res.text)
...
$(document).ready(function () {
    ...
    nationalCertificateIssuance.validateCidSrpNoOnly();
    nationalCertificateIssuance.validateApplicantOthers();
    nationalCertificateIssuance.getCitizenInformation();
    nationalCertificateIssuance.nextTab();
    nationalCertificateIssuance.previousTab();
    nationalCertificateIssuance.save();
});
